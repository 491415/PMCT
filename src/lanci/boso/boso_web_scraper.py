...
import pandas as pd
from boso_utils import clean_naziv, get_all_files
...
urls = get_all_files(datum_cijena)
# Za datoteke u prošlosti
# urls = get_files_past_date(datum_cijena)

for url in urls:
    # Potrebno inače: 'ascii' codec can't encode character '\u0106' in position 52: ordinal not in range(128)
    encoded_url = quote(url, safe=":/?=&,%")

    logging.info(f"Dohvaćam i parsiram datoteku sa URL-a: {encoded_url}...")

    file_name = unquote(encoded_url.split("/")[-1])
    ...
    try:
        ...
        df = pd.read_csv(
            ...
            on_bad_lines="warn",
            encoding=find_encoding(encoded_url),
            engine="python",
        )
        ...
        ducan_id = file_name.split(",")[-4].strip()
        pmpo_id = get_prodajni_objekt_id(db, pmtl_id, ducan_id)

        if not pmpo_id:
            po_dto = ProdajniObjektDTO(
                pmtl_id=pmtl_id,
                pmna_id=get_id_naselja(db, file_name.split(",")[2].title()),
                adresa=file_name.split(",")[1].upper(),
        ...
        df["naziv"] = df["naziv"].apply(clean_naziv)
