...
from src.lanci.dm.dm_utils import (
    clean_marka,
    download_and_save_excel_from_url,
    get_file_link,
    remove_euro_sign,
)
...
url = get_file_link(datum_cijena)
file_name = url.split("/")[-1]
file_name_no_ext = file_name[:-5]  # Briše .xlsx iz naziva datoteke
download_and_save_excel_from_url(url, file_name, file_path)

try:
    logging.info(f"Dohvaćam i parsiram datoteku: {file_name}...")

    podaci = pd.read_excel(url, engine="calamine", sheet_name=None, skiprows=2)
