...
from src.lanci.eurospin.eurospin_utils import clean_naziv
...
datum_cijena = "30.09.2025"
lanac = TrgLanci.EUROSPIN
...
for file_path in path.iterdir():
    ...
    if file_path.is_file():
        ...
        try:
            ...
            ducan_id = file_name.split("-")[1].strip()
            pmpo_id = get_prodajni_objekt_id(db, pmtl_id, ducan_id)

            if not pmpo_id:
                po_dto = ProdajniObjektDTO(
                    pmtl_id=pmtl_id,
                    pmna_id=get_id_naselja(
                        db, file_name.split("-")[3].title().replace("_", " ")
                    ),
                    adresa=file_name.split("-")[2].upper().replace("_", " "),
                    oblik=ProdajniObjektOblikEnum.check_value(
                        file_name.split("-")[0].replace("_", " ").upper()
                    ).value,
                    oznaka=ducan_id,
            ...
            dat_dto = DatotekaDTO(
                pmpr_id=pmpr_id,
                pmpo_id=pmpo_id,
                dat_naziv_zip=f"cjenik_{datum_cijena}-7.30",
                dat_naziv=file_name,
            ...
            df["NAZIV_PROIZVODA"] = df["NAZIV_PROIZVODA"].apply(clean_naziv)
            ...
            if df is not None:
                ...
                for _index, row in df.iterrows():
                    cijene = CijenaDTO(
                        ...
                        cijena_sidrena=Decimal(row.iloc[10]),
                        barkod=str(row.iloc[11]),
                        kategorija=str(row.iloc[12]),
                        datum=datum_cijena,
                    )
