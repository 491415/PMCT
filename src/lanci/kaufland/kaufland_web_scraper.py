...
from src.lanci.kaufland.kaufland_utils import (
    extract_address_city,
    get_all_files,
    remove_strings_in_sidrena_cijena,
)
...
# Ovo je potrebno inače baca grešku za SSL
ssl._create_default_https_context = ssl._create_unverified_context

datum_cijena = datetime.now().strftime(env("DATE_FORMAT"))
lanac = TrgLanci.KAUFLAND
    ...
    try:
        ...
        for col in lanac.stupci_cijena:
            if col in df.columns:
                df[col] = df[col].apply(remove_strings_in_sidrena_cijena)
        ...
        if df is not None:
            ...
            for _index, row in df.iterrows():
                cijene = CijenaDTO(
                    ...
                    barkod=str(row.iloc[13]),
                    kategorija=str(row.iloc[14]),
                    datum=datum_cijena,
                )
