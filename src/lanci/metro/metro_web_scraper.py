...
from src.lanci.metro.metro_utils import create_file_urls, extract_address_city
...
for url in create_file_urls(datum_cijena):
    ...
    try:
        ...
        ducan_id = re.search(r"S\d{2}", file_name).group()
        ...
        if not pmpo_id:
            ...
            po_dto = ProdajniObjektDTO(
                ...
                oblik=ProdajniObjektOblikEnum.check_value(
                    file_name.split("_METRO_")[0].replace("_", " ").upper()
                ).value,
                oznaka=ducan_id,
