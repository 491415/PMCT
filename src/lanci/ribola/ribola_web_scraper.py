...
from src.lanci.ribola.ribola_utils import extract_address_city, get_all_files
...
for url in get_all_files(datum_cijena):
    ...
    ducan_id = re.search(r"-(\d{3})-", file_name)
    ducan_id = ducan_id.group(1)
    ...
    try:
            ...
            if proizvodi_container is not None:
                for proizvod in proizvodi_container.findall("Proizvod"):
                    product_data = {
                        ...
                        "MaloprodajnaCijena": proizvod.find("MaloprodajnaCijena").text,
        ...
        dat_dto = DatotekaDTO(
            ...
            broj_pohrane=file_name.split("-")[-8],
        )
