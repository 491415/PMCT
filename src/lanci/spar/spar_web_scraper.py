...
from src.lanci.spar.spar_utils import extract_address_city, get_all_files
...
for url in get_all_files(datum_cijena):
    ...
    # Mora ovako inače se dobije 403 Forbidden Error
    headers = {
    ...
    try:
        if encoding != "utf-8":
            df = change_file_encoding(
                StringIO(response.text), datum_cijena, file_name, lanac
            )

            pmtl_id = get_tl_id(db, lanac)
            pmpr_id = get_pravilo_id(db, pmtl_id)
            ducan_id = re.search(r"_87\d{2,3}_", file_name).group(0).strip("_")
            ...
            dat_dto = DatotekaDTO(
                ...
                broj_pohrane=file_name.split("_")[-3],
            )
