...
from src.lanci.studenac.studenac_utils import extract_address_city, get_zip_link
...
file_path_cijene = rf"C:\Cijene\{datum_cijena}\{lanac.name}"
create_folders(file_path_cijene)
...
# Ovdje se mora sa 7z jer inače ne radi skripta
with TemporaryDirectory() as temp_dir:
    ...
    try:
        result = subprocess.run(
            [seven_zip, "x", str(zip_path), f"-o{str(temp_path)}", "-y"],
            check=True,
            capture_output=True,
            text=True,
        )
    except subprocess.CalledProcessError as e:
        logging.error(f"Greška prilikom raspakiravanja zip datoteka: {e.stderr}")
    ...
    for file in os.listdir(temp_path):
        ...
        try:
            logging.info(f"Parsiram daototeku: {file_path}...")

            with open(file_path, "rb") as source_file:
                content = source_file.read()

            with open(rf"{file_path_cijene}\{file}", "wb") as xml_file:
            ...
            file_name_no_ext = file[:-4]  # Briše .xml iz naziva datoteke
            pmtl_id = get_tl_id(db, lanac)
            pmpr_id = get_pravilo_id(db, pmtl_id)
            ducan_id = file.split("-")[2]
            pmpo_id = get_prodajni_objekt_id(db, pmtl_id, ducan_id)

            if not pmpo_id:
                adresa, naselje = extract_address_city(file)
            ...
            dat_dto = DatotekaDTO(
                ...
                dat_naziv_zip=zip_link.split("/")[-1][
                    :-4
                ],  # Briše .zip iz naziva datoteke
                dat_format=lanac.file_ext,
                status=StatusEnum.INIT,
                datum_objave=datum_cijena,
                broj_pohrane=file.split("-")[3],
            )
            ...
            logging.error(f"Dogodila se greška za {file_name_no_ext}: {e}")
            raise
