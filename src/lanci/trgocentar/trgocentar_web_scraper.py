...
from src.lanci.trgocentar.trgocentar_utils import extract_address_city, get_all_files
...
for url in get_all_files(datum_cijena):
    ...
    try:
        ...
        for cjenik in root.findall("cjenik"):
            product_data = {
                "naziv_art": cjenik.find("naziv_art").text,
                "sif_art": cjenik.find("sif_art").text,
                "marka": cjenik.find("marka").text,
                "net_kol": cjenik.find("net_kol").text,
                "jmj": cjenik.find("jmj").text,
                "mpc": cjenik.find("mpc").text,
                "c_jmj": cjenik.find("c_jmj").text,
                "mpc_pop": cjenik.find("mpc_pop").text,
                "c_najniza_30": cjenik.find("c_najniza_30").text,
                "c_020525": cjenik.find("c_020525").text,
                "ean_kod": cjenik.find("ean_kod").text,
                "naz_kat": cjenik.find("naz_kat").text,
            }
        ...
        dat_dto = DatotekaDTO(
            ...
            broj_pohrane=file_name.split("_")[-2],
        )
