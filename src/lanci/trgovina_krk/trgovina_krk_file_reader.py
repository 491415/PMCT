...
from src.lanci.trgovina_krk.trgovina_krk_utils import clean_sifra, extract_address_city
...
datum_cijena = ""
lanac = TrgLanci.TRGOVINA_KRK
db = OracleDBConn(lanac.name, run_file=__file__)
db.connect()

inserted_rows = 0

path = Path(rf"C:/Cijene/{datum_cijena}/{lanac.name.replace('_', ' ')}")
    ...
    if file_path.is_file():
        ...
        try:
            ...
            dat_dto = DatotekaDTO(
                ...
                broj_pohrane=file_name.split("_")[-5],
            )
            ...
            df["Šifra proizvoda"] = df["Šifra proizvoda"].apply(clean_sifra)
