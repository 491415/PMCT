...
from src.lanci.zabac.zabac_utils import clean_sifra, get_all_files
...
for url in get_all_files(datum_cijena):
    ...
    ducan_id = "PJ-7"  # Imaju trenutno cijene samo za jedan dućan, ali u nazivu datoteke nema šifre dućana.
    ...
    try:
        ...
        dat_dto = DatotekaDTO(
            ...
            broj_pohrane=file_name_no_ext.split("-")[-1],
        )
        ...
        df["Artikl"] = df["Artikl"].apply(clean_sifra)
        ...
        if df is not None:
            ...
            for _index, row in df.iterrows():
                cijene = CijenaDTO(
                    pmda_id=pmda_id,
                    naziv_proizv=str(row.iloc[4]),
                    sifra_proizv=str(row.iloc[0]),
                    marka_proizv=str(row.iloc[6]),
                    neto_kolicina=str(row.iloc[7]),
                    cijena_mpc=Decimal(row.iloc[5]),
                    cijena_najniza_30=Decimal(row.iloc[8]),
                    cijena_sidrena=Decimal(row.iloc[9]),
                    barkod=str(row.iloc[3]),
                    kategorija=str(row.iloc[6]),
                    datum=datum_cijena,
                )
